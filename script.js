...
window.testDatabase = async function() {
  ...
  try {
    ...
    resultDiv.innerHTML = `✅ Connecté: ${data?.length || 0} spécialité(s) trouvée(s)`;
  } catch (err) {
    ...
    data.forEach(s => {
      html += `
        <div style="background:#f0f0f0; padding:8px; margin:5px 0; border-radius:5px; display:flex; justify-content:space-between; align-items:center;">
          <span>${s.nom}</span>
          <div>
            <button onclick="modifierSpecialite(${s.id}, '${s.nom}')" style="margin-right:5px;">✏️</button>
    ...
    data.forEach(c => {
      html += `
        <div style="background:#f0f0f0; padding:8px; margin:5px 0; border-radius:5px; display:flex; justify-content:space-between; align-items:center;">
          <span>${c.title}</span>
          <div>
            <button onclick="validerCourse(${c.id})" style="margin-right:5px;">✅</button>
