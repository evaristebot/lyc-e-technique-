...
// ===== VÉRIFICATION MOT DE PASSE MASTER =====
...
window.ajouterEleve = function() {
  ...
  alert(`✅ Élève ${nom} ${prenom} ajouté (simulation)`);
...
function loadEleves() {
  ...
  list.innerHTML = `
    <div style="background:#f9f9f9; padding:15px; margin:10px 0; border-radius:10px;">
      <strong>Jean NKOU</strong> - Électricité 1ère<br>
      Parent: 699112233
    ...
    <div style="background:#f9f9f9; padding:15px; margin:10px 0; border-radius:10px;">
      <strong>Marie NGO</strong> - Comptabilité 2ème<br>
      Parent: 699445566
...
window.supprimerEleve = function(id) {
  if(confirm('Supprimer cet élève ?')) {
    alert(`Élève supprimé (simulation)`);
    loadEleves();
  }
};
...
window.ajouterAncien = function() {
  ...
  const parcours = document.getElementById('ancienParcours').value;
  
  alert(`✅ Ancien ${nom} ${prenom} (Bac ${annee}) ajouté`);
...
function loadAnciens() {
  const list = document.getElementById('anciensAdminList');
  list.innerHTML = `
    <div style="background:#f9f9f9; padding:15px; margin:10px 0; border-radius:10px;">
      <strong>Marc TCHANA</strong> - Bac 2015 - Ingénieur
...
window.supprimerAncien = function(id) {
  if(confirm('Supprimer cet ancien élève ?')) {
...
window.rechercher = function() {
  ...
  results.innerHTML = `
    ...
    <div class="result-item">
      ...
      <p><strong>M. Pierre ESSOMBA</strong> - Chef des travaux - Atelier - <a href="https://wa.me/237699112233" target="_blank">WhatsApp</a></p>
      <p><strong>Mme Marie BELL</strong> - Censeur - Français - <a href="https://wa.me/237699445566" target="_blank">WhatsApp</a></p>
    </div>
  `;
};
...
function chargerArticles() {
  ...
  list.innerHTML = `
    <div class="card" style="text-align: left;">
      ...
      <p>La rentrée aura lieu le 9 septembre 2025. Tous les élèves sont attendus.</p>
      <small>Publié le 15/08/2025</small>
    </div>
    <div class="card" style="text-align: left;">
      <h3>Journées Portes Ouvertes</h3>
      <p>Le lycée organise ses JPO les 20-21 novembre. Venez nombreux !</p>
      <small>Publié le 10/08/2025</small>
    </div>
  `;
}
...
function loadArticlesAdmin() {
  ...
  if(list) {
    list.innerHTML = `
      <div style="background:#f9f9f9; padding:15px; margin:10px 0; border-radius:10px;">
        <strong>Rentrée 2025</strong> - Publié le 15/08
        <button onclick="supprimerArticle(1)" style="float:right; background:red; color:white; border:none; padding:5px 10px; border-radius:20px;">🗑️</button>
      </div>
    `;
  }
}
...
// ===== INITIALISATION =====
chargerArticles();
console.log('✅ Site du Lycée Technique de Batouri chargé - Tous les boutons sont actifs');
